  ...
  return {
    ...
    listEmployees: async (ordering, page, page_size) => {
      try {
        const res = await superagent
          .get(
            `${constants.BASE_URL}/${constants.VERSION}/${constants.ME}/${
    ...
    getEmployee: async id => {
      try {
        const res = await superagent
          .get(
            `${constants.BASE_URL}/${constants.VERSION}/${constants.ME}/${constants.EMPLOYEES}/${id}`
          )
          .set("Authorization", `Bearer ${apiKey}`)
          .set("Content-Type", "application/json");
        if (res && res.ok) {
    ...
    getPaymentMethod: async id => {
      try {
        const res = await superagent
          .get(
            `${constants.BASE_URL}/${constants.VERSION}/${constants.ME}/${constants.PAYMENT_METHODS}/${id}`
          )
          .set("Authorization", `Bearer ${apiKey}`)
          .set("Content-Type", "application/json");
        if (res && res.ok) {
